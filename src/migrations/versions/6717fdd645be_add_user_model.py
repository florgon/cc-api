...
def downgrade():
    ...
    sa.Column('view_date', postgresql.TIMESTAMP(), autoincrement=False, nullable=False),
    ...
    sa.ForeignKeyConstraint(['referer_id'], ['referer.id'], name='url_view_referer_id_fkey'),
    sa.ForeignKeyConstraint(['url_id'], ['url.id'], name='url_view_url_id_fkey'),
    sa.ForeignKeyConstraint(['user_agent_id'], ['user_agent.id'], name='url_view_user_agent_id_fkey'),
    sa.PrimaryKeyConstraint('id', name='url_view_pkey')
    )
    op.create_table('user_agent',
    sa.Column('id', sa.INTEGER(), autoincrement=True, nullable=False),
    sa.Column('user_agent_value', sa.VARCHAR(length=4096), autoincrement=False, nullable=False),
